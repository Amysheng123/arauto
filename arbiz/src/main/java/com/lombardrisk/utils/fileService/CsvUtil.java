...
public class CsvUtil {
    ...
    public static List<String> readFile(File file) {
        ...
        try {
            ...
            while ((str = br.readLine()) != null) {
                if (str.contains("/") && str.contains(" 0:00")) {
                    for (String column : str.split(",")) {
                        if ((column.contains("/")) && column.contains(" 0:00")) {
                            String init = column;
                            for (String part : column.split("/")) {
                                if (part.length() == 1)
                                    column = column.replace(part, "0" + part);
                            }
                            column = column.replace(" 0:00", " 00:00:00");
                            str = str.replace(init, column);
                        }
                    }
    ...
    public String getCol(final int index) {
