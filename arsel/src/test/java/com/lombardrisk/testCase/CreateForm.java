...
public class CreateForm extends TestTemplate {
    List<String> Files = createFolderAndCopyFile("CreateForm");
    String testDataFolder = Files.get(0);
    String checkCellFileFolder = Files.get(1);
    File testRstFile = new File(Files.get(2));


    public void CreateNewForm(int ID, String Regulator, String Group, String Form, String ProcessDate, String CopyDataFromExistForm, boolean AllowNull, boolean DeleteExistForm, boolean InitialiseToZeros, String CheckCellValue, String ErrorMessage, String CaseID) throws Exception {
        ...
        try {
            ...
            if ( !ErrorMessage.equals("") ) {
                if ( !listPage.getCreateNewFormErrorMsg(Group, ProcessDate, Form, CopyDataFromExistForm, AllowNull, InitialiseToZeros).equals(ErrorMessage) ) {
                    testRst = false;
                    logger.info("The error message is not expected!");
                }
            ...
            if ( formInstancePage != null && openForm ) {
                if ( CheckCellValue.endsWith("xlsx") ) {
                    ...
                    for (int index = 1; index <= amt; index++) {
                        ...
                        if ( accValue != null ) ExcelUtil.writeToExcel(expectedValueFile, index, 4, accValue);
                        else {
                    ...
                    if ( DeleteExistForm ) {
                        ...
                        Assert.assertEquals(0, adjustLogPage.getLogNums());
            ...
            testRst = false;
            e.printStackTrace();
            logger.error("testCreateNewForm:", e);
            //snapshot( "testCreateFormWithData.png");
    ...
    public void CreateFormFromExcel(int ID, String Regulator, String Group, String Form, String ProcessDate, String CreatefromExcel, boolean AllowNull, boolean DeleteExistForm, boolean InitialiseToZeros, String CheckCellValue, String ErrorMessage, String ErrorInfo, String CaseID) throws Exception {
        ...
        try {
            ...
            File importFile = new File("data\\CreateForm\\Import\\" + CreatefromExcel);
            FormInstancePage formInstancePage = null;
            if ( !ErrorMessage.equals("") ) {
                ...
                File txt = new File("data\\CreateForm\\CheckCellValue\\" + ErrorInfo);
            ...
            if ( formInstancePage != null && openForm ) {
                if ( CheckCellValue.endsWith(".xlsx") ) {
                    ...
                    for (int index = 1; index <= amt; index++) {
                        ...
                        if ( findCell ) {
                            try {
                                if ( Math.abs(Double.parseDouble(accValue)
                                        - Double.parseDouble(expectedValue)) > 0.05 ) {
                                    ExcelUtil.writeToExcel(expectedValueFile, index, 5, "Fail");
                                    testRst = false;
                    ...
                    // Test for allow null and value of CheckCellValue is cell name
                    String cellName = CheckCellValue;
                    String expectedValue = "1";
                    formInstancePage.editCellValue(null, cellName, null, expectedValue);
                    String accValue = formInstancePage.getCellText(Regulator, formCode, version, null, cellName, null);
            ...
            testRst = false;
            e.printStackTrace();
            logger.error("testCreateFormFromExcel:", e);
            //snapshot( "testCreateFormFromExcel.png");
    ...
    public void testCreateForm() throws Exception {
        File testDataFile = new File("data\\CreateForm\\CreateForm.xls");
